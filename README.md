# Skeleton.py
a Claude generated animatronic
...
1. `BehaviorManager.tick()` â€” polls triggers, updates head/jaw servo targets
   from vision/audio, and advances eye brightness.
2. `BehaviorManager.maybe_speak()` â€” occasionally queues a spoken line.
...
| [vision.py](vision.py) | Camera capture + Haar-cascade face tracking â†’ head angle & closeness |
...
| [script_lines.py](script_lines.py) | Spoken-line content (guest lines, event lines, monologues) â€” kept separate from `config.py` |
...
strictly required to run â€” everything else falls back to a mock backend if
...
files are skipped). Copy the resulting `audio/` folder onto the Pi â€”
...
limited CPU (see the module docstring for specifics â€” smaller camera frame,
frame-skipped detection, slower loop rate).

## Gotchas

- `main.py --test` reports in its log output which backend (real or mock)
  was selected for each module â€” if something you've wired up shows "mock",
  that's the first thing to check.
- `generate_audio.py` is meant to run once, off-Pi, before deployment â€” not
